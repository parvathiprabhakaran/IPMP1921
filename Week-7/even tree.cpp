...
void dfs(int n,vector<int>& b,vector<vector<int>>& a,vector<int>& v){
  v[n]=1;int p=0;for(auto it=a[n].begin();it!=a[n].end();it++){
    if(!v[*it]){dfs(*it,b,a,v);p+=b[*it];}
  }b[n]=1+p;return;
}
int evenForest(int n, int e, vector<int> fr, vector<int> to) {
vector<vector<int>>a(n);vector<int>b(n,0);vector<int>v(n,0);for(int i=0;i<e;i++){a[fr[i]-1].push_back(to[i]-1);a[to[i]-1].push_back(fr[i]-1);}
int t=0;dfs(0,b,a,v);
for(int i=0;i<e;i++){int g=min(b[fr[i]-1],b[to[i]-1]);if(g%2==0){t++;}}
return t;
}
